...
create_index_pattern() {
  ...
  # get  ID from $index_pattern
  if [ "$total" -ge 1 ]; then
    existing_id=$(echo "$response" | jq -r --arg pattern "$index_pattern" '
      .saved_objects[] 
      | select(.attributes.title == $pattern) 
      | .id
    ')
    new_ids["$index_pattern"]="$existing_id"
    echo "  ✅ Index pattern '$index_pattern' already exists. ID: ${new_ids[$index_pattern]}"
    return
  fi
...
echo "🔍 Checking if ILM policy '$POLICY_NAME' exists..."

check_response=$(curl -s -o /dev/null -w "%{http_code}" -X GET "$ELASTIC_URL/_ilm/policy/$POLICY_NAME" \
  --cacert "$CA_CERT" \
  -u $USER:$PASS)
...
create_template () {
  ...
  echo "🔍 Checking if ${alias}_template exists..."
  check_response=$(curl -s -o /dev/null -w "%{http_code}" -X GET "$ELASTIC_URL/_index_template/${alias}_template" \
    --cacert "$CA_CERT" \
    -u $USER:$PASS)

  if [ "$check_response" -eq 200 ]; then
    echo " - ✅ ${alias}_template already exists. Skipping creation."
  else
...
echo "============================"
echo
echo "🔍 Check if '$SNAPSHOT_REPOSITORY_NAME' snapshot exists..."

check_response=$(curl -s -o /dev/null -w "%{http_code}" -X GET "$ELASTIC_URL/_snapshot/$SNAPSHOT_REPOSITORY_NAME" \
    --cacert "$CA_CERT" \
    -u $USER:$PASS)

if [ "$check_response" -eq 200 ]; then
  echo " - ✅ $SNAPSHOT_REPOSITORY_NAME already exists. Skipping creation."
else
...
# Crear política 
echo "🔍 Check if '$SNAPSHOT_POLICY_NAME' snapshot policy exists..."
create_response=$(curl -s -w "\n%{http_code}" -X GET "$ELASTIC_URL/_slm/policy/$SNAPSHOT_POLICY_NAME" \
    --cacert "$CA_CERT" \
    -u $USER:$PASS)

if [ "$check_response" -eq 200 ]; then
  echo " - ✅ $SNAPSHOT_POLICY_NAME already exists. Skipping creation."
else
