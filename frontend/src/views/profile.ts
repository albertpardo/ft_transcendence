...
export async function renderProfileContent(el: HTMLElement) {
      ...
      <div class="flex flex-col md:flex-row gap-12">
        ...
        <div class="md:w-1/3 flex flex-col items-center space-y-6">
          <div class="bg-gray-800 p-6 rounded-lg w-full flex flex-col items-center">
            ...
            <button id="edit-btn"
                    class="mt-6 px-4 py-2 bg-yellow-500 text-white text-base rounded-lg hover:bg-yellow-600 transition-colors">Edit
            </button>
          </div>
        </div>
          ...
          <form id="profile-form" class="space-y-6">
            ...
            <div>
              <button type="submit" id="save-btn"
                      class="px-6 py-3 bg-green-700 text-white rounded-lg hover:bg-green-600 transition" disabled>
                Save Changes
              </button>
            </div>
            <div>
              <button type="submit" id="delete-btn"
                      class="px-6 py-3 bg-red-800 text-white rounded-lg hover:bg-red-600 transition" disabled>
                Delete Account
              </button>
            </div>
          </form>
  ...
  editBtn.addEventListener("click", () => {
    inputs.forEach((input) => {
      ...
      // Resaltar que los campos son editables con un efecto visual
      if (input.type !== "file") {
        input.select(); // Selecciona el texto para facilitar la edición
      }
    });
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        // Update the display name and nickname in the UI
