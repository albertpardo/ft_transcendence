    ...
    switch (event.data) {
      case "connected":
        ...
        case "added: L":
          started = false;
          playerSide = "l";
          if (leftUpArrow) leftUpArrow.hidden = false;
          if (leftDownArrow) leftDownArrow.hidden = false;
          if (rightUpArrow) rightUpArrow.hidden = true;
          if (rightDownArrow) rightDownArrow.hidden = true;
          gameText!.style.visibility = "hidden";
          scoreText!.classList.add('opacity-0');
          setTimeout(() => {
...
export async function initDashboard() {
  ...
  const leftUpArrow: HTMLElement | null = document.getElementById("left-up");
  const leftDownArrow: HTMLElement | null = document.getElementById("left-down");
  const rightUpArrow: HTMLElement | null = document.getElementById("right-up");
  const rightDownArrow: HTMLElement | null = document.getElementById("right-down");
  const ball: HTMLElement | null = document.getElementById("ball");
  const lpad: HTMLElement | null = document.getElementById("lpad");
  const rpad: HTMLElement | null = document.getElementById("rpad");
...
//          console.log("Welcome to pong.");
        ...
        case "added: L":
          ...
          if (rightUpArrow) rightUpArrow.hidden = true;
          if (rightDownArrow) rightDownArrow.hidden = true;
          if (gameText) gameText.style.visibility = "hidden";
          scoreText.classList.add('opacity-0');
            setTimeout(() => {
              scoreText.innerHTML = `${gameState.stateScoreL} : ${gameState.stateScoreR}`;
        ...
        case "added: R":
          started = false;
          playerSide = "r";
          if (rightUpArrow) rightUpArrow.hidden = false;
          if (rightDownArrow) rightDownArrow.hidden = false;
          if (leftUpArrow) leftUpArrow.hidden = true;
          if (leftDownArrow) leftDownArrow.hidden = true;
          if (gameText) gameText.style.visibility = "hidden";
          scoreText.classList.add('opacity-0');
            setTimeout(() => {
              scoreText.innerHTML = `${gameState.stateScoreL} : ${gameState.stateScoreR}`;
        ...
        default:
          const newState: State =JSON.parse(event.data);
           ...
           if (ball) ball.setAttribute("cx","" + newState.stateBall.coords.x);
           if (ball) ball.setAttribute("cy","" + newState.stateBall.coords.y);
           if (lpad) lpad.setAttribute("y",  "" + newState.stateLP.y);
           if (rpad) rpad.setAttribute("y",  "" + newState.stateRP.y);
          /*  try {
          ...
          if (gameState.stateWhoL !== "none" && gameState.stateWhoL !== "null state") {
            if (gameText) gameText.style.visibility = "visible";
            if (gameText) gameText.classList.remove('opacity-0');
            // gameText.classList.remove('animate-win-pulse', 'animate-lose-pulse', 'animate-text-glow');
            ...
            if (playerSide === "l") {
              switch (gameState.stateWhoL) {
                case "left":
                  if (gameText) gameText.innerHTML = "You lost the round.";
                  if (gameText) gameText.classList.remove('fill-white');
                  if (gameText) gameText.setAttribute("fill", "#f87171");
                  break;
                case "right":
                  if (gameText) gameText.innerHTML = "You won the round!";
                  if (gameText) gameText.classList.remove('fill-white');
                  if (gameText) gameText.setAttribute("fill", "#4ade80");
                  break;
                case "left fully":
                  started = false;
                  if (gameText) gameText.innerHTML = "You lost the game.";
                  if (gameText) gameText.classList.remove('fill-white');
                  if (gameText) gameText.setAttribute("fill", "#f87171");
                  setTimeout(() => triggerRainEffect(), 300);
                  break;
                case "right fully":
                  started = false;
                  if (gameText) gameText.innerHTML = "You won the game!";
                  if (gameText) gameText.classList.remove('fill-white');
                  if (gameText) gameText.setAttribute("fill", "#4ade80");
                  setTimeout(() => triggerConfetti(), 300);
                  break;
            ...
            if (scoreText) scoreText.innerHTML = "" + gameState.stateScoreL + " : " + gameState.stateScoreR;
...
//            console.log(result);
    ...
    if (leftUpArrow) leftUpArrow.addEventListener('mousedown', () => {
      movePaddleWrapper(-2);
    });

    if (leftUpArrow) leftUpArrow.addEventListener('mouseup', () => {
      movePaddleWrapper(0);
    });

    if (leftUpArrow) leftUpArrow.addEventListener('mouseleave', () => {
      movePaddleWrapper(0);
    });

    if (leftDownArrow) leftDownArrow.addEventListener('mousedown', () => {
      movePaddleWrapper(2);
    });

    if (leftDownArrow) leftDownArrow.addEventListener('mouseup', () => {
      movePaddleWrapper(0);
    });

    if (leftDownArrow) leftDownArrow.addEventListener('mouseleave', () => {
      movePaddleWrapper(0);
    });

    if (rightUpArrow) rightUpArrow.addEventListener('mousedown', () => {
      movePaddleWrapper(-2);
    });

    if (rightUpArrow) rightUpArrow.addEventListener('mouseup', () => {
      movePaddleWrapper(0);
    });

    if (rightUpArrow) rightUpArrow.addEventListener('mouseleave', () => {
      movePaddleWrapper(0);
    });

    if (rightDownArrow) rightDownArrow.addEventListener('mousedown', () => {
      movePaddleWrapper(2);
    });

    if (rightDownArrow) rightDownArrow.addEventListener('mouseup', () => {
      movePaddleWrapper(0);
    });

    if (rightDownArrow) rightDownArrow.addEventListener('mouseleave', () => {
      movePaddleWrapper(0);
    });
